...
class _NewNotesScreenState extends State<NewNotesScreen> {
  ...
  Future<void> _loadNote() async {
    final doc = quill.Document()..insert(0, 'Tap to add a Note');
    setState(() {
        ...
        onKey: (event) {
          if (event.data.isControlPressed && event.character == 'b') {
            if (_controller!
                ...
                .contains('bold')) {
              _controller!
                  .formatSelection(quill.Attribute.clone(quill.Attribute.bold, null));
            } else {
  ...
  Widget _buildWelcomeEditor(BuildContext context) {
    var quillEditor = quill.QuillEditor.basic(
