...
class PostsCommentsModel with ChangeNotifier {
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadpostcomments),
          body: jsonEncode({
            "data": {"id": 0, "post": post}
          }));
      if (response.statusCode == 200) {
        ...
        List<Comments> comments = await compute(parseComments as FutureOr<List<Comments>> Function(String), response.body);
  ...
  Future<void> fetchMoreComments() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Map<String, dynamic> res = json.decode(response.body);
        hasMoreComments = res["has_more"];
        List<Comments> articles = await compute(parseComments as FutureOr<List<Comments>> Function(String), response.body);
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.makepostcomment),
  ...
  Future<void> deleteComment(int? commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.deletepostcomment),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int? id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.editpostcomment),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.reportpostcomment),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
