...
class FollowPeople extends StatefulWidget {
  static const routeName = "/FollowPeople";
  FollowPeople({this.check});
  final bool? check;
...
class FollowPeopleRouteState extends State<FollowPeople> {
  List<Userdata>? items = [];
  ...
  Future<void> fetchItems() async {
    ...
    try {
      ...
      dio.interceptors.add(
        DioCacheManager(
          CacheConfig(baseUrl: ApiUrl.getUsersToFollow),
        ).interceptor,
      );
            ...
            onPressed: () {
              if (widget.check!) {
                Navigator.pushReplacementNamed(
                    context, SocialActivity.routeName);
              } else {
