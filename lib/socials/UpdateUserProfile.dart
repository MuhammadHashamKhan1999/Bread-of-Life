...
class UpdateUserProfileState extends State<UpdateUserProfile> {
  ...
  pickImages(String type, {quantity = 1}) async {
    try {
      _mediaPaths = await ImagePicker().pickImage(
          maxHeight: 300,
          maxWidth: 300, source: ImageSource.gallery);
    } on PlatformException {
      Alerts.show(context, t.error, t.couldnotprocess);
    }
    if (!mounted) return;
    convertPath(_mediaPaths.toString(), type);
  }

  convertPath(String uri, String type) async {
    print("convert file");
    final filePath = await LecleFlutterAbsolutePath.getAbsolutePath( uri: uri);
    if (type == "avatar") {
