...
class RepliesModel with ChangeNotifier {
  ...
  Future<void> fetchComments() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Replies> comments = await compute(parseComments as FutureOr<List<Replies>> Function(String), response.body);
  ...
  Future<void> fetchMoreComments() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Map<String, dynamic> res = json.decode(response.body);
        hasMoreComments = res["has_more"];
        List<Replies> articles = await compute(parseComments as FutureOr<List<Replies>> Function(String), response.body);
  ...
  Future<void> deleteComment(int? commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response =
          await http.post(Uri.parse(ApiUrl.deletereply), body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
        Map<String, dynamic> res = json.decode(response.body);
  ...
  Future<void> editComment(int? id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response =
          await http.post(Uri.parse(ApiUrl.editreply), body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
        Map<String, dynamic> res = json.decode(response.body);
