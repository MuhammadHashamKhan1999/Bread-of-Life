  ...
  Future<void> fetchItems() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Media> mediaList = await compute(parseSliderMedia as FutureOr<List<Media>> Function(String), response.body);
        if (page == 0) {
          if (subCategoriesList.length == 0) {
            subCategoriesList = await compute(parseCategories as FutureOr<List<Categories>> Function(String), response.body);
            addTopItem();
          }
          setItems(mediaList);
        } else {
