...
class DownloadsModel with ChangeNotifier {
  ...
  Downloads? isMediaInDownloads(int? id) {
    Downloads? itm =
        downloadsList.firstWhereOrNull((itm) => itm.id == id);
    return itm;
  }
  ...
  void requestDownload(Downloads task) async {
    ...
    final itm = downloadsList.firstWhereOrNull((itm) => itm.taskId == task.taskId);
    if (itm != null) {
  ...
  void _bindBackgroundIsolate() {
    ...
    _port.listen((dynamic data) {
      ...
      if (downloadsList.length > 0) {
        ...
        final task = downloadsList.firstWhereOrNull((task) => task.taskId == id);
        if (task != null) {
          task.status = status;
